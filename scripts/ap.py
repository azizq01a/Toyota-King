...
def find_telemetry_files(data_dir):
    ...
    for root, dirs, files in os.walk(data_dir):
        ...
        if "__MACOSX" in root:
            continue
        for f in files:
            if "telemetry_data.csv" in f and not f.startswith("._"):
...
def find_lap_files(data_dir):
    ...
    for root, dirs, files in os.walk(data_dir):
        if "__MACOSX" in root:
            continue
        for f in files:
            if "_lap_time.csv" in f and not f.startswith("._"):
...
for t_file, l_file in zip(telemetry_files, lap_files):
    ...
    try:
        telemetry = pd.read_csv(t_file, encoding='utf-8', errors='ignore')  # تجاهل مشاكل الترميز
    except Exception as e:
        print("[SIM] Failed to read:", t_file, e)
        continue
    ...
    for i, row in telemetry.iterrows():
        payload = {col: row[col] if col in row else None for col in ["driver", "speed", "lap", "throttle", "brake", "tire_wear", "fuel", "timestamp"]}
        try:
